  ...
  while True:
    ...
    for event in events:
      if event["type"] == "message" and "text" in event: # check for message events
        ...
        if(first[-2:] == "++"):
          if(len(message) == 1):
            sendMess("{thing} just got props!".format(thing=first.encode("utf-8"))) 
          else:
            sendMess("{thing} just got props {reason}!".format(thing=first.encode("utf-8"), reason=message[1].encode("utf-8")))
